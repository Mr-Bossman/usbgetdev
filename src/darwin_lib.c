...
#define DARWIN_LOCATION_LEN 256
static int darwin_get_location(struct libusb_device *dev, UInt32 *location) {
	int ret;
	char *path;
	long int loc;

	path = malloc(DARWIN_LOCATION_LEN);

	if(!path)
		return LIBUSB_ERROR_NO_MEM;

	ret = libusb_get_platform_device_id(dev, path, DARWIN_LOCATION_LEN);
	if (ret != LIBUSB_SUCCESS) {
		free(path);
		return ret;
	}

	loc = strtoul(path, NULL, 10);
	free(path);

	if (loc == 0 || loc > UINT32_MAX)
...
*/
	...
	kresult = darwin_get_location (dev, &location);
	if (kresult != kIOReturnSuccess)
		return LIBUSB_ERROR_NOT_FOUND;
